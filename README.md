...
2. Run `npx wrangler login --scopes account:read user:read workers_scripts:write ai:write` and complete Cloudflare's authorization in the browser. Wrangler also requests background access to refresh its login. Credentials stay in Wrangler's local configuration, never in this repository or the frontend.
